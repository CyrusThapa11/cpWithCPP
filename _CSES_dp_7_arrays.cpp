	...
	frvar(idxx, 1, n + 1, 1) {
		...
		if (array[idxx] == 0) {
			...
			if (idxx == n - 1) {
				...
				// -1
				if ( abs(array[idxx - 1] - 1  -  array[idxx + 1]) <= 1 ) {
					currCount++;
				}
				// + 0
				if ( abs(array[idxx - 1]  -  array[idxx + 1]) <= 1) {
					currCount++;
				}
				// +1
				if ( abs(array[idxx - 1] + 1  -  array[idxx + 1]) <= 1) {
			...
			if ( idxx != n - 1 && array[idxx + 1] != 0  ) {
				if (abs(array[idxx] - array[idxx + 1]) > 1 ) {
					cout << 0;
					return;
				}
			}
